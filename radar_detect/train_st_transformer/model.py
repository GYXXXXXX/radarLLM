"""Lightweight patch embedding + spatial-temporal Transformer model."""
...
class FmcwPatchSTTransformer(nn.Module):
    """Scene-window model with objectness and trajectory heads.

    Input shape:
        [B, 2*Nrx, Tin, Nchirp, Nfast]

    Output:
        objectness_logits: [B, K]
        state_pred:        [B, K, Tout, 4]
    """
    ...
    def __init__(
        ...
        self.nfast = int(nfast)
        self.patch_chirp = int(patch_chirp)
        self.patch_fast = int(patch_fast)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        state_pred = self.state_head(slot_features).view(
            bsz,
            self.max_targets,
            self.tout,
            4,
        )
        return objectness_logits, state_pred
